...
const BUCKET_NAME = 'auth.dobugs.co.kr';
const RSYNC_EXCLUDE =
  '\
  --exclude=build \
...
function updateVersion() {
  return new Promise(async (resolve, reject) => {
      ...
      const questions = [
        {
          name: 'updateVersion',
          type: 'input',
          message: `Update Version (${packageJSON.version}) :`,
        },
      ];
    ...
    const version = answers.updateVersion || packageJSON.version;

    packageJSON.version = version;
    manifestJSON.version = version;
    manifestJSON['minimum-version'] = version;
...
function deployToDevelopment() {
  deployToAWS('development', 'build:dev', 'E3GCZVQI8FWLGC');
}

function deployToProduction() {
  deployToAWS('production', 'build:prod', 'E1QC74Z25PB0KE');
}
